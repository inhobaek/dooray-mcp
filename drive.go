...
func DriveTools(s *server.MCPServer, token *string) {
	tool := mcp.NewTool("dooray_drive",
		mcp.WithDescription("list Dooray drives and folders. 'find_drives' (needs queryString, e.g. 'type=project&scope=public'), 'find_folders' (needs driveId, queryString, e.g. 'type=folder&subTypes=root')."),
		mcp.WithString("operation",
			mcp.Required(),
			mcp.Description("The operation to perform."),
			mcp.Enum("find_drives", "find_folders"),
		),
		...
		mcp.WithString("driveId",
			mcp.Description("drive id. Required for find_folders."),
		),
	)
